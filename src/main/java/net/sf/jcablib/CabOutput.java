...
public class CabOutput {
    ...
    private Vector              mCabFolders     = new Vector();
    ...
    private short getTotalFiles() {
        if(mTotalFiles == -1) {
            ...
            Iterator i = mCabFolders.iterator();
            Iterator iFiles;
            while(i.hasNext()) {
                totalFolders++;
                iFiles = ((CabFolder)i.next()).iterator();
    ...
    private int getHeaderSize() {
        ...
        for(int i=0; i < mTotalFolders; i++) {
            currentFolder = (CabFolder)mCabFolders.get(i);
     ...
     * @throws java.io.IOException
     */
    private void writeCabinetHeader()
    throws IOException {
        
        int     cab_header_checksum = 0,
                cab_file_size       = 0,
                cab_folder_checksum = 0,
                cab_entry_offset    = 0,
                cab_file_checksum   = 0;
        short   cab_version         = 0,
                cab_folders         = getTotalFolders(),
                cab_files           = getTotalFiles(),
     ...
     * @param e
     */
    public void putNextEntry(CabEntry e)
    throws CabException {
     ...
     * @param f
     */
    public void putNextFolder(CabFolder f)
    throws CabException {
        currentFolder = f;
    }
